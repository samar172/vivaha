...
router.post("/book", asyncHandler(async (req, res) => {
  ...
  const order = await prisma.$transaction(async (tx) => {
    ...
    const o = await tx.order.create({ data: { id, customerId: c.id, status: "BOOKED", subtotal: cv.totals.taxable, tax: cv.totals.tax, total: cv.totals.total, requiredBy: requiredBy ? new Date(requiredBy) : new Date(Date.now() + 14 * 864e5), holdUntil: new Date(Date.now() + line.holdMins * 60_000), bookedBy: req.user!.name, source: "portal", lines: { create: lineData }, events: { create: { from: null, to: "BOOKED", by: req.user!.name, why: "Booked via wholesale portal" } } } });
