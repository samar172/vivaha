...
const lineSchema = z.object({ itemId: z.string(), qty: z.number().int().positive(), rate: z.number().min(0), alloc: z.record(z.number().int().min(0)), batchNo: z.string().optional(), expiry: z.string().optional() });
...
router.post("/:id/receive", requirePerm("purchase.create"), asyncHandler(async (req, res) => {
  const b = z.object({ lines: z.array(z.object({ itemId: z.string(), alloc: z.record(z.number().int().min(0)), batchNo: z.string().optional() })) }).parse(req.body);
  ...
  await prisma.$transaction(async (tx) => {
    for (const l of po.lines) {
      ...
      await tx.purchaseLine.update({ where: { id: l.id }, data: { alloc: inp.alloc, batchNo: inp.batchNo ?? null } });
    }
