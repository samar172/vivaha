...
export default function InvoicesPage() {
  ...
  return <>
    <PageHead
      ...
      actions={<button className="b b-o" onClick={() => exportCsv("invoices", ["Invoice", "Date", "Line", "Firm", "GSTIN", "Order", "Taxable", "CGST", "SGST", "IGST", "Total"], rows.map((r) => [r.no, fDate(r.date), r.line?.name ?? "—", r.customer.name, r.customer.gstin ?? "Unregistered", r.orderId, r.taxable, r.cgst, r.sgst, r.igst, r.total]))}><Icon n="download" s={13} /> Export</button>}
    />
    <div className="wa">
      <div className="kpis">
      ...
      <div className="gw"><table className="dg"><thead><tr>
        ...
        <th className="n">Taxable</th><th className="n">Tax</th><th className="n">Total</th><th>Status</th>
      </tr></thead><tbody>
        ...
        </tr>) : <tr><td colSpan={8}><Empty t="No invoices" d={line === "ALL" ? "No invoice has been raised yet. One is raised automatically when an order is dispatched." : `No invoice on the ${L?.name ?? "selected"} line for this search.`} /></td></tr>}
      </tbody></table></div>
