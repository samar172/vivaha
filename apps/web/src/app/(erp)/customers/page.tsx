...
export default function CustomersPage() {
  ...
  return <>
    <PageHead crumb={["Sales", "Customers"]} title="Customers" sub={`${rows.length} firms · ledger, credit gate and machine profile per firm`} actions={<><button className="b b-o" onClick={() => exportCsv("customers", ["Firm", "Contact", "Tehsil", "Group", "GSTIN", "Limit", "Outstanding", "Oldest days", "Gate"], rows.map((c) => [c.name, c.contactName, c.tehsil, c.group, c.gstin, c.creditLimit, c.gate.out, c.gate.oldestAge, c.gate.restricted ? "Restricted" : "OK"]))}>⤓ Export</button>{can("cust.edit") && <button className="b b-p" onClick={() => openModal(<CustomerForm />, "w")}>+ New customer</button>}</>} />
    <div className="wa">
      <div className="tbar"><div className="tsr">🔍<input placeholder="Firm, contact, tehsil or GSTIN…" value={q} onChange={(e) => setQ(e.target.value)} /></div><button className="b b-o b-s" onClick={() => tg("gate")}>Past credit gate</button><button className="b b-o b-s" onClick={() => tg("blocked")}>Blocked</button><button className="b b-o b-s" onClick={() => tg("machines")}>Has machines</button>{filters.map((f) => <span className="chip" key={f}>{f}<span className="x" onClick={() => tg(f)}>✕</span></span>)}</div>
