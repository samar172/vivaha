...
import { money, fDate, dueLbl, daysTo, orderProgress, type OrderStatus } from "@vivaha/shared";
...
export default function OrdersPage() {
  ...
  const [tab, setTab] = useState("approve"); const [q, setQ] = useState(""); const [sel, setSel] = useState<Set<string>>(new Set()); const [sort, setSort] = useState<{ k: string; d: 1 | -1 } | null>(null);
  ...
  let rows = data?.orders ?? []; if (sort && tab !== "approve") rows = rows.slice().sort((a, b) => ((sort.k === "total" ? a.total - b.total : new Date(a.requiredBy).getTime() - new Date(b.requiredBy).getTime()) * sort.d));
  ...
  return <>
    <PageHead crumb={["Sales", "Orders"]} title="Orders" sub="Booking → approval → reservation → allocation → pick → pack → dispatch. Lapsed holds keep the intent; partial dispatch creates a backorder." actions={<button className="b b-o" onClick={() => exportCsv("orders", ["Order", "Firm", "Status", "Lines", "Taxable", "Tax", "Total", "Required by", "Created"], rows.map((o) => [o.id, o.customer.name, o.status, o.lines.length, o.subtotal, o.tax, o.total, fDate(o.requiredBy), fDate(o.createdAt)]))}>⤓ Export</button>}
      tabs={[{ k: "approve", l: "Awaiting approval", n: cnt.approve }, { k: "active", l: "In fulfilment", n: cnt.active }, { k: "shipped", l: "Shipped", n: cnt.shipped }, { k: "closed", l: "Closed", n: cnt.closed }, { k: "all", l: "All", n: cnt.all }]} tab={tab} onTab={(k) => { setTab(k); setSel(new Set()); }} />
    ...
    <div className="wa">
      <div className="tbar"><div className="tsr">🔍<input placeholder="Order number or firm…" value={q} onChange={(e) => setQ(e.target.value)} /></div>{tab === "approve" && <span style={{ fontSize: 11, color: "var(--t4)" }}>Sorted by hold remaining — the order about to lapse is always first</span>}<span style={{ marginLeft: "auto", fontSize: 11, color: "var(--t4)" }}>{rows.length} shown</span></div>
      <div className="gw"><table className="dg"><thead><tr><th style={{ width: 30 }}></th>{tab === "approve" && <th className="n">Hold left</th>}<th>Order</th><th>Firm</th><th>Lines</th><th className="n sortable" onClick={() => sortBy("total")}>Value{ic("total")}</th><th className="n sortable" onClick={() => sortBy("req")}>Required by{ic("req")}</th><th>Credit</th><th>Status</th><th>Progress</th><th></th></tr></thead><tbody>
        {pg.rows.length ? pg.rows.map((o) => { const c = o.customer, g = o.gate; const rd = daysTo(o.requiredBy), urgent = rd >= 0 && rd <= 7 && !["DELIVERED", "DISPATCHED", ...CLOSED].includes(o.status); const closed = CLOSED.includes(o.status);
          return <tr key={o.id} className={(sel.has(o.id) ? "sel" : "") + (closed ? " dim" : "")} onClick={() => openDrawer(<OrderDrawer id={o.id} />)}>
            ...
            <td>{fDate(o.requiredBy)}<div className="sm" style={{ color: urgent ? "var(--er)" : "var(--t4)" }}>{dueLbl(o.requiredBy)}{urgent ? " ⚠" : ""}</div></td>
