  ...
  return <>
    ...
    <div className="wa"><Note k="w" style={{ marginBottom: 11 }}>Printing cannot start without a recorded proof approval — this is the single largest source of disputes in the trade.</Note>
      <div className="gw"><table className="dg"><thead><tr><th>Job</th><th>Firm</th><th>Base card</th><th>Process</th><th className="n">Qty</th><th className="n">Quote</th><th>Required by</th><th>Proofs</th><th>Status</th><th></th></tr></thead><tbody>
        {rows.map((j) => { const rd = daysTo(j.requiredBy); return <tr key={j.id} onClick={() => router.push(`/jobs/${j.id}`)}><td><span className="rid">{j.id}</span><div className="sm">{fDate(j.createdAt)}</div></td><td className="w">{j.customer.name}</td><td className="w">{itemRef(j.baseItem)}<div className="sm">{j.baseItem.name}</div></td><td className="w">{j.processItem.name}</td><td className="n tab">{num(j.qty)}</td><td className="n tab">{money(j.quote)}</td><td style={{ color: rd <= 7 ? "var(--er)" : "inherit" }}>{fDate(j.requiredBy)}<div className="sm">{dueLbl(j.requiredBy)}</div></td><td className="tab">{j.proofs}</td><td><Pill s={j.status} text={JOB_STATUS_LABEL[j.status]} /></td><td>{j.status === "PROOF_SENT" ? <button className="b b-o b-s" onClick={(e) => { e.stopPropagation(); toast("Waiting on customer approval in portal", "i"); }}>Awaiting approval</button> : j.status === "QUOTED" ? <button className="b b-o b-s" onClick={(e) => { e.stopPropagation(); toast("Quote resent on WhatsApp", "s"); }}>Resend quote</button> : null}</td></tr>; })}
      </tbody></table></div></div>
  </>;
}
