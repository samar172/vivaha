...
function JobDrawer({ id }: { id: string }) {
  ...
  const move = async (to: JobStatus) => { try { await post(`/api/jobs/${j.id}/status`, { to, godownId: to === "PRINTING" && gd ? gd : undefined }); toast(`${j.id} → ${JOB_STATUS_LABEL[to]}`, "s"); closeDrawer(); refresh("/api/"); } catch (e) { toast(errMsg(e), "e"); } };
  ...
  return <DrawerFrame onClose={closeDrawer} head={<><span className="rid" style={{ fontSize: 14.5 }}>{j.id}</span><Pill s={j.status} text={JOB_STATUS_LABEL[j.status]} /></>} actions={next ? <button className="b b-p b-s" onClick={() => move(next)}>{label[next] ?? JOB_STATUS_LABEL[next]}</button> : null}>
