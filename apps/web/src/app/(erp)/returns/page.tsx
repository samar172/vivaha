...
export function ReturnDetail({ id }: { id: string }) {
  ...
  return <>
    <PageHead
      ...
      sub={<>{r.qty} {r.item.uom} of {r.item.sku} off {r.orderId} · raised {fDate(r.createdAt)}{r.resolvedAt ? ` · closed ${fDate(r.resolvedAt)}` : ""}</>}
      actions={<>
            ...
            <Section t="What came back">
              <div style={{ display: "flex", gap: 13, alignItems: "flex-start", marginBottom: 13, cursor: "pointer" }} onClick={() => router.push(`/items/${r.item.id}`)}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div className="sm"><span className="rid">{r.item.sku}</span> · <LineChip id={r.item.lineId} /></div>
                </div>
