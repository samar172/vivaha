  ...
  return <>
    ...
    <div className="wa">
      ...
      <div className="g3"><div>
        ...
        <Panel t="Fastest moving" h={<span className="pnmore" onClick={() => router.push("/reports")}>Velocity report →</span>}><div className="pnb" style={{ padding: "8px 13px" }}>{d.fastest.map((x) => <div key={x.item.id} className="crow" style={{ padding: "6px", borderBottom: "1px solid var(--bd-soft)", borderRadius: 0, gap: 9 }} title={`Open ${x.item.sku}`} onClick={() => openDrawer(<ItemDrawer id={x.item.id} />)}><ZoomThumb it={x.item} w={40} h={52} zoom={280} style={{ width: 32 }} caption={<><b>{x.item.name}</b><br />{x.item.designNo ? x.item.designNo + " · " : ""}{x.item.sku} · {num(x.item.available)} {x.item.uom?.toLowerCase()} available</>} /><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{x.item.name}</div><div className="sm">{x.item.sku} · avail {num(x.item.available)}</div></div><div className="tab" style={{ fontSize: 13, fontWeight: 700 }}>{num(x.qty)}</div></div>)}</div></Panel>
