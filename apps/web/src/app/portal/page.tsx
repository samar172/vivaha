...
import { money, num, itemRefOr } from "@vivaha/shared";
    ...
    <div className="blk">
      {back.map((w) => <div className="altrow" key={w.id}>
        ...
        <div className="in">
          ...
          <div className="sm">{itemRefOr(w.item)}</div>
...
export default function PortalHome() {
  ...
  const doScan = async () => { const r = await apiFetch<PItem[]>(`/api/portal/scan?q=__random__&line=${P.line}`); if (r[0]) { toast(`Scanned ${itemRefOr(r[0])}`, "s"); P.openSheet(r[0].id); } };
