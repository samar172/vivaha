...
function ItemSheet({ id }: { id: string }) {
  const P = usePortal(); const { toast } = useUI();
  ...
  return <>
    <div className="shh"><button className="b b-g b-s" onClick={P.closeSheet}>←</button><span className="rid" style={{ fontSize: 13 }}>{it.designNo || it.sku}</span><LineChip id={it.lineId} /><button className="b b-g b-s" style={{ marginLeft: "auto" }} onClick={P.closeSheet}>✕</button></div>
    <div className="shb">
      ...
      <div className="blk"><div style={{ fontSize: 16, fontWeight: 700, lineHeight: 1.35 }}>{it.name}</div><div className="hi" style={{ fontSize: 13.5, color: "var(--t6)", marginTop: 2 }}>{it.nameHi}</div><div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: 9 }}>{Object.values(it.attrs).map((a) => <span className="atc" key={a}>{a}</span>)}</div></div>
      ...
      <div className="blk"><div className="lb">आपका रेट</div><div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 9 }}><span style={{ fontSize: 26, fontWeight: 700 }}>₹{pr.rate}</span><span style={{ fontSize: 12.5, color: "var(--t6)" }}>प्रति {it.uom.toLowerCase()}</span><span style={{ marginLeft: "auto", fontSize: 11.5, color: "var(--t4)" }}>MOQ {num(it.moq)}</span></div>
      ...
      {short && q > 0 && <div className="blk" style={{ borderColor: "var(--er-bd)" }}><div className="lb" style={{ color: "var(--er)" }}>सिर्फ़ {num(b.qty)} उपलब्ध — {num(q)} चाहिए</div>{b.qty > 0 && <button className="b b-o hi" style={{ width: "100%", marginBottom: 11 }} onClick={() => setQ(b.qty)}>{num(b.qty)} बुक करें</button>}<div className="lb">ये विकल्प देखें</div>
        {data.alternates.length ? data.alternates.map((a) => <div className="altrow" key={a.item.id}><img className="th cw" src={thumb(a.item, 56, 72)} alt="" style={{ border: "1px solid var(--bd)", borderRadius: 4 }} /><div className="in"><div style={{ fontSize: 12.5, fontWeight: 600 }}>{a.item.name}</div><div className="sm">{a.item.designNo || a.item.sku} · {Object.values(a.item.attrs).slice(0, 2).join(" · ")}</div><div style={{ marginTop: 4, fontSize: 12.5 }}><b>₹{a.rate}</b> · <span style={{ color: a.avail >= q ? "var(--ok)" : "var(--wa)" }}>{num(a.avail)} उपलब्ध</span></div></div><button className="b b-o b-s hi" onClick={() => P.openSheet(a.item.id, q)}>देखें</button></div>) : <div className="sm hi">इस समय कोई विकल्प उपलब्ध नहीं</div>}
        {data.alternates.length > 0 && b.qty > 0 && <div className="split"><div style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 6 }} className="hi">या मिला-जुला लें</div><div style={{ fontSize: 12.5, color: "var(--t6)" }}>{num(b.qty)} × {it.designNo || it.sku} + {num(q - b.qty)} × {data.alternates[0].item.designNo || data.alternates[0].item.sku} = {num(q)} {it.uom.toLowerCase()}</div><button className="b b-p hi" style={{ width: "100%", marginTop: 9 }} onClick={() => split(data.alternates[0].item, b.qty, q - b.qty)}>दोनों बुक करें</button></div>}</div>}
    </div>
    <div className="shf"><div style={{ flex: 1 }}><div className="sm">कुल</div><div style={{ fontSize: 17, fontWeight: 700 }} className="tab">{money(pr.rate * Math.min(q, short ? b.qty : q))}</div></div>{b.canBook ? <button className="b b-p hi" style={{ height: 44, padding: "0 26px", fontSize: 15 }} onClick={book}>{short ? "उपलब्ध बुक करें" : "बुक करें"}</button> : <button className="b b-o hi" style={{ height: 44, padding: "0 20px" }} onClick={() => { toast("हम आपको सूचित करेंगे", "s"); P.closeSheet(); }}>स्टॉक आने पर बताएं</button>}</div>
  </>;
}

function CartSheet() {
  const P = usePortal(); const { toast } = useUI(); const router = useRouter(); const [req, setReq] = useState(() => new Date(Date.now() + 14 * 864e5).toISOString().slice(0, 10)); const [busy, setBusy] = useState(false);
  ...
  return <>
    <div className="shh"><strong style={{ fontSize: 14 }} className="hi">आपकी बुकिंग</strong><button className="b b-g b-s" style={{ marginLeft: "auto" }} onClick={P.closeSheet}>✕</button></div>
    <div className="shb">
      {Object.keys(cart.byLine).map((lid) => { const L = P.me!.lines.find((l) => l.id === lid); const gl = cart.lines.filter((l) => l.item.lineId === lid); const blk = cart.totals.blocks.find((b) => b.gstPct === cart.byLine[lid].gstPct); return <div className="lgrp" key={lid}><div className="hd"><LineChip id={lid} /><span style={{ marginLeft: "auto", color: "var(--t4)", fontWeight: 500 }}>{gl.length} आइटम</span></div><div className="bd2">
        {gl.map((l) => <div className="crow" key={l.item.id}><img className="th cw" src={thumb(l.item, 48, 62)} alt="" style={{ border: "1px solid var(--bd)", borderRadius: 4 }} /><div className="in"><div style={{ fontSize: 12.5, fontWeight: 600 }}>{l.item.name}</div><div className="sm">{l.item.designNo || l.item.sku} · ₹{l.rate}/{l.item.uom.toLowerCase()}</div><div style={{ display: "flex", alignItems: "center", gap: 9, marginTop: 6 }}><div className="qs"><button onClick={() => P.addLine(l.item.id, -l.item.moq)}>−</button><span className="tab">{num(l.qty)}</span><button onClick={() => P.addLine(l.item.id, l.item.moq)}>+</button></div><b className="tab" style={{ fontSize: 13 }}>{money(l.amount)}</b><button className="b b-g b-s" style={{ marginLeft: "auto", color: "var(--t4)" }} onClick={() => P.rmLine(l.item.id)}>हटाएं</button></div></div></div>)}
    ...
    <div className="shf">{g.restricted && c.gateMode === "BLOCK" ? <button className="b b-o hi" style={{ flex: 1, height: 44 }} onClick={() => toast(`ऑफ़िस को कॉल किया जा रहा है — ${c.salesExec?.name ?? "सेल्स"}`, "i")}>ऑफ़िस से बात करें</button> : <button className="b b-p hi" style={{ flex: 1, height: 44, fontSize: 15 }} disabled={busy} onClick={place}>बुकिंग कन्फ़र्म करें</button>}</div>
  </>;
}
