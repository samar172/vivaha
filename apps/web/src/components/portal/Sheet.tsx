...
function ItemSheet({ id }: { id: string }) {
  ...
  // The quantity is held as text while it is being typed, so clearing the box
  // leaves it empty instead of snapping to 0 and fighting the next keystroke.
  const [qText, setQText] = useState("");
  const q = Math.max(0, Number(qText) || 0);
  const [init, setInit] = useState(false);
  // The quantity is part of the request, so it only follows once typing stops.
  // Sending it per keystroke made a new SWR key each time, and a key with an
  // empty cache blanked the whole sheet — taking the input, and the caret in
  // it, with it. That is what threw the operator out mid-number.
  const qDebounced = useDebounced(q, 400);
  const { data } = useApi<SheetData>(`/api/portal/items/${id}?qty=${qDebounced}`, { keepPreviousData: true });
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { if (data && !init) { setQText(String(P.sheetQty || data.item.moq)); setInit(true); } }, [data, init, P.sheetQty]);
  ...
  return <>
    ...
    <div className="shb">
      ...
      {b.canBook && <div className="blk"><div className="lb">मात्रा</div><div className="qty"><button onClick={() => setQText(String(Math.max(0, q - it.moq)))}>−</button><input type="number" inputMode="numeric" min={0} value={qText} onChange={(e) => setQText(e.target.value.replace(/[^0-9]/g, ""))} onFocus={(e) => e.target.select()} /><button onClick={() => setQText(String(q + it.moq))}>+</button></div><div className="sm hi" style={{ marginTop: 6 }}>{num(it.moq)} के गुणक में · {it.packUom} में {num(it.perPack)}</div><div className="lb" style={{ marginTop: 12 }}>कब चाहिए</div><input type="date" className="scani" value={req} onChange={(e) => setReq(e.target.value)} style={{ marginTop: 0 }} /></div>}
      {short && q > 0 && <div className="blk" style={{ borderColor: "var(--er-bd)" }}><div className="lb" style={{ color: "var(--er)" }}>सिर्फ़ {num(b.qty)} उपलब्ध — {num(q)} चाहिए</div>{b.qty > 0 && <button className="b b-o hi" style={{ width: "100%", marginBottom: 11 }} onClick={() => setQText(String(b.qty))}>{num(b.qty)} — {t("book")}</button>}<div className="lb">{t("seeAlternatives")}</div>
...
// Stepping by the minimum order quantity is fine for a box or two and hopeless
// for two thousand, so the number can be typed as well. It is committed when
// the box is left or Enter is pressed, not per keystroke — each commit is a
// request, and the server is the one that decides what the cart holds.
function CartQty({ line }: { line: { item: PItem; qty: number } }) {
  const P = usePortal();
  // A draft only exists while the box is being edited; the rest of the time the
  // cart is simply the truth. Deriving it this way means there is no state to
  // keep in step with the server, and no effect to do the keeping.
  const [draft, setDraft] = useState<string | null>(null);
  const text = draft ?? String(line.qty);

  const commit = () => {
    const n = Math.max(0, Number(text) || 0);
    setDraft(null);
    if (n === line.qty) return;
    if (n === 0) { P.rmLine(line.item.id); return; }
    P.addLine(line.item.id, n, "set");
  };
  return <div className="qs">
    <button onClick={() => P.addLine(line.item.id, -line.item.moq)}>−</button>
    <input
      className="tab" type="number" inputMode="numeric" min={0} value={text}
      onFocus={(e) => { setDraft(String(line.qty)); e.target.select(); }}
  ...
  return <>
    ...
    <div className="shb">
      {Object.keys(cart.byLine).map((lid) => { const L = P.me!.lines.find((l) => l.id === lid); const gl = cart.lines.filter((l) => l.item.lineId === lid); const blk = cart.totals.blocks.find((b) => b.gstPct === cart.byLine[lid].gstPct); return <div className="lgrp" key={lid}><div className="hd"><LineChip id={lid} /><span style={{ marginLeft: "auto", color: "var(--t4)", fontWeight: 500 }}>{gl.length} आइटम</span></div><div className="bd2">
        {gl.map((l) => <div className="crow" key={l.item.id}><img className="th cw" src={thumb(l.item, 48, 62)} alt="" style={{ border: "1px solid var(--bd)", borderRadius: 4 }} /><div className="in"><div className={lang === "hi" ? "hi" : ""} style={{ fontSize: 14, fontWeight: 600 }}>{itemName(l.item, lang)}</div><div className="sm">{l.item.designNo || l.item.sku} · ₹{l.rate}/{l.item.uom.toLowerCase()}</div><div style={{ display: "flex", alignItems: "center", gap: 9, marginTop: 6 }}><CartQty line={l} /><b className="tab" style={{ fontSize: 14.5 }}>{money(l.amount)}</b><button className="b b-g b-s" style={{ marginLeft: "auto", color: "var(--t4)" }} onClick={() => P.rmLine(l.item.id)}>{t("remove")}</button></div></div></div>)}
