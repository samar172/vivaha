...
function AmendBillModal({ inv, orderId }: { inv: AmendInvoice; orderId: string }) {
  const { closeModal, toast } = useUI();
  const [rows, setRows] = useState(() => inv.lines.map((l) => ({ id: l.id, sku: l.sku, itemName: l.itemName, gstPct: l.gstPct, shipped: l.shipped ?? l.qty, wasQty: l.qty, wasRate: l.rate, qty: l.qty, rate: l.rate })));
  ...
  return <ModalFrame title={"Correct bill " + inv.no} onClose={closeModal}
    ...
    <table className="dg" style={{ marginBottom: 12, fontSize: 13 }}><thead><tr><th>Item</th><th className="n">Shipped</th><th className="n">Billed qty</th><th className="n">Rate</th><th className="n">Amount</th></tr></thead><tbody>
      {rows.map((r, i) => <tr key={r.id} style={{ cursor: "default" }}>
        <td className="w">{r.itemName}<div className="sm">{r.sku}{(Number(r.qty) !== r.wasQty || Number(r.rate) !== r.wasRate) && <> · was {num(r.wasQty)} × {money2(r.wasRate)}</>}</div></td>
        <td className="n tab">{num(r.shipped)}</td>
        <td className="n"><input type="number" min={1} max={r.shipped} value={r.qty} onChange={(e) => set(i, { qty: Number(e.target.value) || 0 })} style={{ width: 88, height: 28, border: "1px solid " + (Number(r.qty) > r.shipped ? "var(--er)" : "var(--bd)"), borderRadius: 5, padding: "0 7px", textAlign: "right" }} /></td>
        <td className="n"><input type="number" value={r.rate} onChange={(e) => set(i, { rate: Number(e.target.value) || 0 })} style={{ width: 92, height: 28, border: "1px solid var(--bd)", borderRadius: 5, padding: "0 7px", textAlign: "right" }} /></td>
