...
export const NAV: { g: string; items: { k: string; l: string; i: string; perm: Perm }[] }[] = [
  { g: "Overview", items: [{ k: "dashboard", l: "Dashboard", i: "▦", perm: "dash.view" }] },
  { g: "Catalogue", items: [{ k: "items", l: "Items & Rates", i: "🏷", perm: "item.view" }] },
  { g: "Supply", items: [{ k: "purchase", l: "Purchase & GRN", i: "📥", perm: "purchase.view" }, { k: "stock", l: "Inventory", i: "🏬", perm: "stock.view" }] },
  { g: "Sales", items: [{ k: "customers", l: "Customers", i: "🧑", perm: "cust.view" }, { k: "orders", l: "Orders", i: "🧾", perm: "order.view" }, { k: "dispatch", l: "Dispatch", i: "🚚", perm: "order.dispatch" }, { k: "jobs", l: "Job Work", i: "🖨", perm: "order.view" }, { k: "returns", l: "Returns", i: "↩", perm: "return.view" }] },
  { g: "Finance", items: [{ k: "accounts", l: "Ledger & GST", i: "₹", perm: "ledger.view" }] },
  { g: "Insight", items: [{ k: "reports", l: "Reports", i: "📈", perm: "report.view" }, { k: "audit", l: "Audit Log", i: "🕵", perm: "audit.view" }] },
  { g: "System", items: [{ k: "settings", l: "Settings", i: "⚙", perm: "settings.manage" }] },
];
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return (
    <FootCtx.Provider value={ctx}>
      <div className="erp">
        <div className="gh">
          <button className="gi" style={{ display: "none" }} id="mobMenu" onClick={() => setMob((m) => !m)}>☰</button>
          ...
          <button className="gsel" onClick={() => setPal("godown ")}>📍 <span>{gdLabel}</span> ▾</button>
          <div className="dbadge">DEMO</div>
          <button className="gsr" onClick={() => setPal("")}>🔍 <span className="lbl">Search orders, customers, SKUs…</span><kbd>⌘K</kbd></button>
          <button className="gi" onClick={() => setNt((v) => !v)} title="Notifications">🔔{!!notifs?.unread && <span className="bg">{notifs.unread}</span>}</button>
          <div className="rch">👤 <span>{ROLE_LABELS[user.role]}</span></div>
        ...
        <div className="shell">
          <aside className={"sb" + (sbCol ? " col" : "") + (mob ? " mob" : "")}>
            <nav className="sbn">{NAV.map((g) => { const its = g.items.filter((i) => can(i.perm)); if (!its.length) return null; return <div className="ng" key={g.g}><div className="nl">{g.g}</div>{its.map((i) => { const c = navCount(i.k); const on = pathname.startsWith("/" + i.k); return <Link href={"/" + i.k} key={i.k} className={"ni" + (on ? " on" : "")}><span className="ic">{i.i}</span><span>{i.l}</span>{c ? <span className="ct">{c}</span> : null}</Link>; })}</div>; })}</nav>
            <button className="sbc" onClick={toggleSb}>{sbCol ? "▸" : "◂"}</button>
          </aside>
      ...
      <div className={"dr" + (nt ? " on" : "")} style={{ width: 370 }}>
        <div className="drh"><strong style={{ fontSize: 13 }}>Notifications</strong><button className="b b-g b-s" style={{ marginLeft: "auto" }} onClick={() => post("/api/notifications/read-all").then(() => mutNotifs())}>Mark all read</button><button className="b b-g b-s" onClick={() => setNt(false)}>✕</button></div>
        <div className="drb">{notifs?.notifications.length ? notifs.notifications.map((n) => <div className="ds" key={n.id} style={{ opacity: n.isRead ? .55 : 1, cursor: n.link ? "pointer" : "default" }} onClick={() => { post(`/api/notifications/${n.id}/read`).then(() => mutNotifs()); if (n.link) { setNt(false); router.push(n.link); } }}><div style={{ display: "flex", gap: 9 }}><span style={{ color: `var(--${({ WARN: "wa", OK: "ok", ERR: "er", INFO: "in" } as Record<string, string>)[n.kind]})` }}>{({ WARN: "⚠", OK: "✓", ERR: "✕", INFO: "ℹ" } as Record<string, string>)[n.kind]}</span><div><div style={{ fontSize: 12, lineHeight: 1.5 }}>{n.text}</div><div className="sm" style={{ marginTop: 3 }}>{fDT(n.createdAt)}</div></div></div></div>) : <div className="empty"><div className="t">Nothing new</div><div className="d">You are all caught up.</div></div>}</div>
      </div>
  ...
  return (
    <div className="pv on" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="pl"><input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Type a command, order number, SKU or customer…" />
        <div className="plr">
          {nav.length > 0 && <><div className="plg">Go to</div>{nav.slice(0, 6).map((i) => <div className="pli" key={i.k} onClick={() => go("/" + i.k)}><span className="ic">{i.i}</span>{i.l}</div>)}</>}
          {ql.startsWith("godown") && <><div className="plg">Godown</div>{[{ id: "ALL", name: "All godowns" }, ...godowns].map((g) => <div className="pli" key={g.id} onClick={() => setGodown(g.id)}><span className="ic">📍</span>{g.name}</div>)}</>}
          {data?.orders.length ? <><div className="plg">Orders</div>{data.orders.map((o) => <div className="pli" key={o.id} onClick={() => go(`/orders?open=${o.id}`)}><span className="ic">🧾</span>{o.id} — {o.firm}<kbd>{o.status.replace(/_/g, " ")}</kbd></div>)}</> : null}
          {data?.items.length ? <><div className="plg">Items</div>{data.items.map((i) => <div className="pli" key={i.id} onClick={() => go(`/items?open=${i.id}`)}><span className="ic">🏷</span>{i.sku} — {i.name}<kbd>{num(i.available)}</kbd></div>)}</> : null}
          {data?.customers.length ? <><div className="plg">Customers</div>{data.customers.map((c) => <div className="pli" key={c.id} onClick={() => go(`/customers?open=${c.id}`)}><span className="ic">🧑</span>{c.name}<kbd>{money(c.outstanding)}</kbd></div>)}</> : null}
