...
export function OrderDrawer({ id }: { id: string }) {
  ...
  void blocks;
  return <DrawerFrame onClose={closeDrawer} head={<><span className="rid" style={{ fontSize: 13 }}>{o.id}</span><Pill s={o.status} /></>}
    ...
    {o.holdUntil && <Section style={{ background: "var(--wa-bg)" }}><div style={{ display: "flex", alignItems: "center", gap: 9, fontSize: 12.5, color: "var(--wa)", fontWeight: 600 }}>⏳ Hold expires in <span style={{ fontSize: 15 }}><Hold until={o.holdUntil} onExpire={() => setTimeout(() => mutate(), 16000)} /></span><span style={{ marginLeft: "auto", fontWeight: 500 }}>then stock releases and an alert is raised</span></div></Section>}
...
function AllocModal({ o }: { o: Order }) {
  ...
  return <ModalFrame title={"Godown allocation — " + o.id} onClose={closeModal} actions={<><button className="b b-o" onClick={closeModal}>Cancel</button><button className="b b-p" onClick={save}>Confirm allocation</button></>}>
    ...
    {o.lines.map((l) => <div key={l.id} style={{ border: "1px solid var(--bd)", borderRadius: 6, padding: 10, marginBottom: 9 }}><div style={{ fontSize: 12, fontWeight: 700, marginBottom: 7 }}>{l.item.sku} — {l.item.name} · need {num(l.qty)} {l.item.uom}</div><div style={{ display: "flex", gap: 8 }}>{godowns?.map((g) => { const av = (items?.items.find((i) => i.id === l.itemId)?.godowns.find((x) => x.godownId === g.id)?.available ?? 0) + (l.alloc[g.id] || 0); return <div key={g.id} style={{ flex: 1 }}><div className="sm" style={{ marginBottom: 3 }}>{g.short} · avail {num(av)}</div><input type="number" value={al[l.itemId]?.[g.id] ?? 0} onChange={(e) => setAl({ ...al, [l.itemId]: { ...al[l.itemId], [g.id]: Number(e.target.value) || 0 } })} style={{ width: "100%", height: 30, border: "1px solid var(--bd)", borderRadius: 5, padding: "0 7px" }} /></div>; })}</div><div className="sm" style={{ marginTop: 6 }}>{sum(l.itemId) === l.qty ? <span style={{ color: "var(--ok)", fontWeight: 700 }}>✓ {num(sum(l.itemId))} / {num(l.qty)} allocated</span> : <span style={{ color: "var(--er)", fontWeight: 700 }}>✕ {num(sum(l.itemId))} / {num(l.qty)} — must equal the ordered quantity</span>}</div></div>)}
  </ModalFrame>;
}
